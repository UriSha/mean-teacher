...
def errors(logits, labels, name=None):
    ...
    Assume unlabeled examples have label == [0, 0, 0, ... , 0].
    ...
    with tf.name_scope(name, "errors") as scope:
        ...
        per_sample = tf.losses.mean_squared_error(labels, logits)
...
def classification_costs(logits, labels, name=None):
    ...
    Assume unlabeled examples have label == [0, 0, 0, ... , 0] . For unlabeled examples, cost == 0.
    ...
    with tf.name_scope(name, "classification_costs") as scope:
        ...
        per_sample = tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits, labels=labels)
